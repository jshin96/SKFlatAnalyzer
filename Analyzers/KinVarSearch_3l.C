...
void KinVarSearch::executeEvent(){
  ...
  if(SS3l){
    MakeTreeSS3L(muonTightColl, muonLooseColl, muonVetoColl, electronTightColl, electronLooseColl, electronVetoColl,
                 jetColl, bjetColl, vMET_T1xy, weight, "");
  }

}

void KinVarSearch::MakeTreeSS3L(vector<Muon>& MuTColl, vector<Muon>& MuLColl, vector<Muon>& MuVColl,
                                ...
                                vector<Jet>& JetColl, vector<Jet>& BJetColl, Particle& vMET, float weight, TString Label)
{
  ...
  if( !( (NMuT==2 and NElT==1) or (NElT==2 and NMuT==1) ) ) return;
